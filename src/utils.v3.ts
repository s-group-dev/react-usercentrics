import type { ConsentStatusFromLocalStorage, ServiceId, UCDataFromLocalStorage, UCWindow } from './types.v3.js'
 ...
 */
export const areAllConsentsAccepted = async (): Promise<boolean> => {
    if (typeof window !== 'undefined') {
        const consentDetails = await (window as UCWindow).__ucCmp?.getConsentDetails()
        return consentDetails?.consent.status === 'ALL_ACCEPTED'
    }
